...
contract SampleMultipleTypeNFTTest is Test {
    ...
    function testInitialState() public view {
        ...
        assertEq(nft.minTokenType(), MIN_TOKEN_TYPE);
        assertEq(nft.maxTokenType(), MAX_TOKEN_TYPE);
    }
    ...
    function testSetMaxTokenTypeAndFreezeTokenTypeRange() public {
        ...
        nft.setMaxTokenType(MAX_TOKEN_TYPE);

        // setMaxTokenType: success
        vm.prank(owner);
        nft.setMaxTokenType(MAX_TOKEN_TYPE + 1);

        assertEq(nft.minTokenType(), MIN_TOKEN_TYPE);
        assertEq(nft.maxTokenType(), MAX_TOKEN_TYPE + 1);
    ...
    function testSetBaseTokenURI() public {
        ...
        nft.airdropByType(holder1, MIN_TOKEN_TYPE);
        nft.airdropByType(holder2, MAX_TOKEN_TYPE);
        vm.stopPrank();
    ...
    function testAirdropByType() public {
        ...
        for (uint256 tt = MIN_TOKEN_TYPE; tt <= MAX_TOKEN_TYPE; tt++) {
            ...
            }
            {
                ...
                nft.tokenOfOwnerByIndex(holder1, tt - MIN_TOKEN_TYPE);
            }
        }
        ...
        for (uint256 tt = MIN_TOKEN_TYPE; tt <= MAX_TOKEN_TYPE; tt++) {
            ...
            }
            {
                ...
                nft.tokenURI(tt - MIN_TOKEN_TYPE);
            }
            {
                vm.expectRevert(
                    abi.encodeWithSelector(
                ...
                nft.tokenType(tt - MIN_TOKEN_TYPE);
            }
            assertEq(nft.typeSupply(tt), 0);
            assertEq(nft.typeBalanceOf(holder1, tt), 0);
            {
                vm.expectRevert(
                    abi.encodeWithSelector(
                        IERC721Errors.ERC721NonexistentToken.selector,
                ...
                nft.firstOwnerOf(tt - MIN_TOKEN_TYPE);
            }
            {
                vm.expectRevert(
                    abi.encodeWithSelector(
                ...
                nft.holdingPeriod(tt - MIN_TOKEN_TYPE);
            }
            {
                vm.expectRevert(
                    abi.encodeWithSelector(
                ...
                nft.royaltyInfo(tt - MIN_TOKEN_TYPE, 0);
            }
            {
                vm.expectRevert(
                    abi.encodeWithSelector(
            ...
            }
            {
                ...
                nft.userExpires(tt - MIN_TOKEN_TYPE);
            }
        }
        ...
        for (uint256 tt = MIN_TOKEN_TYPE; tt <= MAX_TOKEN_TYPE; tt++) {
            ...
            assertEq(nft.tokenURI(tt - MIN_TOKEN_TYPE), "");
            assertEq(nft.tokenType(tt - MIN_TOKEN_TYPE), tt);
            assertEq(nft.typeSupply(tt), 1);
            assertEq(nft.typeBalanceOf(holder1, tt), 1);
            assertEq(nft.firstOwnerOf(tt - MIN_TOKEN_TYPE), holder1);
            assertEq(nft.holdingPeriod(tt - MIN_TOKEN_TYPE), 0);
            {
            ...
            assertEq(nft.userOf(tt - MIN_TOKEN_TYPE), ZERO_ADDRESS);
            assertEq(nft.userExpires(tt - MIN_TOKEN_TYPE), 0);
        }
        ...
        for (uint256 tt = MIN_TOKEN_TYPE; tt <= MAX_TOKEN_TYPE; tt++) {
            ...
            nft.airdropByType(ZERO_ADDRESS, tt);
        }
        vm.stopPrank();
        ...
        for (uint256 tt = MIN_TOKEN_TYPE; tt <= MAX_TOKEN_TYPE; tt++) {
            assertEq(nft.holdingPeriod(tt - MIN_TOKEN_TYPE), 1 days);
        }
    }
    ...
    function testSafeTransferFromHolder1ToHolder2() public {
        ...
        assertEq(nft.tokenURI(0), tokenURI);
        assertEq(nft.tokenType(0), MIN_TOKEN_TYPE);
        assertEq(nft.typeSupply(MIN_TOKEN_TYPE), 1);
        assertEq(nft.typeBalanceOf(holder1, MIN_TOKEN_TYPE), 1);
        assertEq(nft.typeBalanceOf(holder2, MIN_TOKEN_TYPE), 0);
        ...
        assertEq(nft.tokenType(0), MIN_TOKEN_TYPE);
        assertEq(nft.typeSupply(MIN_TOKEN_TYPE), 1);
        assertEq(nft.typeBalanceOf(holder1, MIN_TOKEN_TYPE), 0);
        assertEq(nft.typeBalanceOf(holder2, MIN_TOKEN_TYPE), 1);
        assertEq(nft.firstOwnerOf(0), holder1);
    ...
    function testBurn() public {
        ...
        assertEq(nft.typeSupply(MIN_TOKEN_TYPE), 0);
        assertEq(nft.typeBalanceOf(holder1, MIN_TOKEN_TYPE), 0);
        assertEq(nft.firstOwnerOf(0), holder1);
        {
            vm.expectRevert(
